...
# Gompertz model
# this model assumes the same underlying population model at each site
# observation error is associated with differences between grids at the same site

# missing values for seed are modelled at the site level
# specify the rainfall lag for the process model in the data statement
...
# run the model in JAGS with three chains
  mod.jags <- jags(model = "model.txt",
              data = list(Y = Y, Ytau = Ytau, seed = nseed, nGrids = nGrids, nYears = nYears, lag = lag.sp, td = td),
