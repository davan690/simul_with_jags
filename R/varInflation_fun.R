## Model function for looking at variance effect
## Possible simulation paper
## Code written March2020

## JAGS model
## replicated over `nYears` and `nGrids`
##
varInflation <- 300

effect_model_varInflation <- function(varInflation = varInflation){
  ...
  mod <- "model {
   ...
   for(i in 1:nYears) {
     for(j in 1:nGrids) {
       ...
       Y[i, j] ~ dnorm(X[i, j], Ytau[i, j] + varInflation)
...
mod <- effect_model_varInflation(varInflation = varInflation)
mod

# write model
write(mod, "./R/model_test_fun.txt")
